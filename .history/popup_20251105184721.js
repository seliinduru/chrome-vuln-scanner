// popup.js - NİHAİ, SEKME GEÇİŞİ DÜZELTİLMİŞ SON VERSİYON
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderAiSuggestions = (vulns) => {
        ...
        vulns.forEach(vuln => {
            ...
            switch (vuln.type) {
                case 'xss':
                    suggestionsHTML += `<div class="ai-advice"><h4>XSS Zafiyetleri İçin</h4><p>Kullanıcıdan gelen verileri DOM'a yazdırırken <strong>textContent</strong> kullanın. Eğer HTML yazdırmak zorundaysanız, bilinen bir "DOMPurify" gibi bir kütüphane ile veriyi temizleyin.</p></div>`;
                    suggestionMap.xss = true;
                    break;
                case 'csp':
                    suggestionsHTML += `<div class="ai-advice"><h4>CSP Eksikliği İçin</h4><p>HTTP başlıklarına katı bir <strong>Content-Security-Policy</strong> ekleyin. Bu, XSS saldırılarının etkisini büyük ölçüde azaltır.</p></div>`;
    ...
    const renderVulns = () => {
        if (!vulnList || !fuzzyLogicConfig || !scoreMethods) {
            if(vulnList) vulnList.innerHTML = `<div class="empty-state"><p>Modeller yüklenemedi.</p></div>`;
            return;
        }
    ...
    // DÜZELTİLMİŞ SEKME DEĞİŞTİRME MANTIĞI
    document.querySelectorAll('.tab').forEach(tabButton => {
        tabButton.addEventListener('click', (event) => {
            const clickedTabId = event.currentTarget.dataset.tab;
    
            // Ayarlar sekmesine tıklandığında özel işlem yap
            if (clickedTabId === 'settings') {
                window.location.href = 'settings.html';
                return; 
            }
    
            // 1. Tüm sekme butonlarından 'active' sınıfını kaldır
            document.querySelectorAll('.tab').forEach(btn => btn.classList.remove('active'));
    
            // 2. Sadece tıklanan sekme butonuna 'active' sınıfını ekle
            event.currentTarget.classList.add('active');
    
            // 3. Tüm içerik alanlarını gizle ('active' sınıfını kaldırarak)
            document.querySelectorAll('.tab-content').forEach(content => {
                content.classList.remove('active');
            });
    
            // 4. Sadece tıklanan sekmeye ait içerik alanını göster
            const targetContent = document.getElementById(clickedTabId);
            if (targetContent) {
                targetContent.classList.add('active');
            }
    
            // 5. Eğer tıklanan sekme "AI" ise, önerileri o anda oluştur
            if (clickedTabId === 'ai') {
