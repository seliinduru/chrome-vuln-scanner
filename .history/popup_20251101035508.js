...
function showVulnDetails(vuln) {
    ...
    listContainer.style.style.display = 'none';
    detailsContainer.style.style.display = 'block';
...
async function startScan() {
    ...
    try {
        ...
        // 1. content.js'nin yüklenip yüklenmediğini kontrol et ve gerekirse enjekte et.
        let contentReady = false;
        try {
            const statusResponse = await chrome.tabs.sendMessage(tab.id, { action: 'checkStatus' });
            if (statusResponse?.status === 'ready') contentReady = true;
        } catch (e) { /* Mesaj gönderilemedi, content.js enjekte edilmeli */ }
        ...
        if (!contentReady) {
            ...
            // Enjeksiyondan sonra bir kez daha kontrol et (asenkron olduğu için)
            try {
                const afterInjectStatus = await chrome.tabs.sendMessage(tab.id, { action: 'checkStatus' });
                if (afterInjectStatus?.status !== 'ready') throw new Error("Content script injection failed.");
            } catch (e) {
                console.error('Content script injection/check failed:', e);
                throw new Error("Content script injection failed or failed to respond.");
            }
        }
