...
(function() {
    ...
    // Kodun bu izole ortamda daha önce çalışıp çalışmadığını kontrol et.
    // SyntaxError'ı kalıcı olarak çözmek için bu gereklidir.
    if (window.CONTENT_SCRIPT_RUNNING) {
        // Yalnızca durum kontrolü yanıtı verilir, ancak tüm kod bloğu tekrar çalışmaz.
    ...
    // Değişkenler artık IIFE'ye özgüdür ve tekrar tanımlama hatası yapmaz.
    var vulnerabilities = []; 
    ...
    function checkCSP() {
      try {
        ...
        const cspHeader = document.querySelector('meta[http-equiv="X-Content-Security-Policy" i]'); 
    ...
    // Message listener'ın IIFE içinde olması ve CONTENT_SCRIPT_RUNNING ile korunması, 
    // her yeniden enjeksiyonda sadece bir kez eklenmesini sağlar.
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      // 1. Tarama komutu
