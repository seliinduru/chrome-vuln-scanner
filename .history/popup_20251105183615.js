// popup.js - AI ÖNERİLERİ AYRI SAYFAYA TAŞINMIŞ NİHAİ VERSİYON
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const FuzzyLogicEngine = {
        evaluate: (vuln, config) => {
            ...
            const fuzzyInputs = { type: {}, location: {} };
            for(const term in memberships.type){ fuzzyInputs.type[term] = (term === vuln.type) ? 1.0 : 0.0; }
            for(const term in memberships.location){ fuzzyInputs.location[term] = (term === vuln.location) ? 1.0 : 0.0; }
    ...
    document.querySelectorAll('.tab').forEach(tab => {
        tab.addEventListener('click', (e) => {
            ...
            // Sadece sonuçlar sekmesi için class değiştir
