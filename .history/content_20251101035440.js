...
// DÜZELTME 1: 'let' yerine 'var' kullanıldı (Tekrar tanımlama hatasını çözmek için)
var vulnerabilities = []; 
// DÜZELTME 2: 'let' yerine 'var' kullanıldı (Tekrar tanımlama hatasını çözmek için)
var networkMonitorInstalled = false; // Ağ monitörünün kurulu olup olmadığını takip et
...
function checkPasswordFields() {
  try {
    ...
    passwordFields.forEach((field, index) => {
      ...
      if (form) {
        // form.action'ın varlığını ve string olup olmadığını kontrol et.
        const formAction = form.action;
        
        if (formAction && typeof formAction === 'string') {
          // Form HTTPS üzerinden gönderilmiyor
          if (formAction.startsWith('http://')) {
            addVuln(`form_insecure_password_${index}`, `Şifre Formu Güvensiz Bağlantı`, `Şifre formu HTTP üzerinden gönderiliyor: ${formAction.slice(0, 80)}`, 'transport', 'critical', { formAction: formAction }, 'form');
          }
        }
        // Eğer action boşsa, sayfanın kendi URL'ine gönderilir. Sayfa HTTP ise bu yine bir risktir.
        else if (window.location.protocol === 'http:') {
          addVuln(`form_insecure_password_${index}_page`, `Şifre Formu Güvensiz Sayfada`, `Şifre formu HTTP sayfasında kullanılıyor. Formun action'ı tanımlanmamış olsa bile sayfa HTTP olduğu için risklidir.`, 'transport', 'critical', { formAction: window.location.href }, 'form');
        }
      }
    });
...
function installNetworkMonitor() {
  // networkMonitorInstalled artık var olduğu için if koşulu bu hatayı önler.
...
// Popup veya background'tan gelen mesajları dinle
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  // 1. Tarama komutu
  if (request.action === 'scanPage') {
    scanPage();
    // Tarama sonuçlarını background.js'e de göndermemiz gerekiyor (scanPage içinde yapılıyor)
    // Mesajlaşma asenkron olduğu için, tarama tamamlandıktan sonra hemen yanıt gönderilebilir.
    sendResponse({ vulnerabilities });
    return true; 
  }
  
  // 2. Durum kontrolü (popup.js'teki injection mekanizması için)
