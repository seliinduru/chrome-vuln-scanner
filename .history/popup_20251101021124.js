...
// ===== STATE VARIABLES =====
let currentVulns = []; // HATA ÇÖZÜMÜ: Başlangıçta boş dizi olarak tanımlandı
...
function executeFuzzyLogic(vulnData, llmModel = 'gpt') {
  ...
  if (model.rules && Array.isArray(model.rules)) {
    model.rules.forEach(rule => { // <<< DÜZELTME: model.rules üzerinde döngü yapılıyor
      let activation = 1;
...
// Tarama başlatma (İletişim/Race Condition hatası çözüldü)
if (scanBtn) {
...
if (exportJsonBtn) {
  exportJsonBtn.addEventListener('click', () => {
    if (!currentVulns || !currentVulns.length) {
...
if (exportCsvBtn) {
  exportCsvBtn.addEventListener('click', () => {
    // HATA ÇÖZÜMÜ: currentVulns'ın null/undefined olmadığından emin ol
    if (!currentVulns || !currentVulns.length) {
      alert('Dışa aktarmak için önce tarama yapın.');
      return;
    }
...
// CSV dönüştürücü (HATA ÇÖZÜMÜ: arr'nin varlığı kontrol edildi)
function toCsv(arr) {
  if (!arr || !arr.length) return ''; // arr'nin null/undefined olup olmadığı kontrol edildi
